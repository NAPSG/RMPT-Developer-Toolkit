  ...
  return dojoDeclare([dijitWidgetBase, dijitTemplatedMixin, dijitWidgetsInTemplate], {
    ...
    parentStateDidChange: function (info) {
      ...
      if (info.isActive) {
        ...
        //If parent widget is closed, close the coordinate format dialog if visible
    ...
    postCreate: function () {
      ...
      // Included this hack to accommodate for issue with BiDi brackets for RTL behavior
      // source:https://stackoverflow.com/questions/5801820/how-to-solve-bidi-bracket-issues
      if (isRTL && this.type === "UTM (H)") {
    ...
    cpSubBtnWasClicked: function (evt) {
      ...
      // Select coordinates to copy
      this.selectText(this[c]);
      // Execute copy to clipboard
      var result = this.execCopy();
      // Show message if copy was successful
      this.showToolTip(evt.currentTarget.id, result ? this.nls.copySuccessful : this.nls.copyFailed);
    ...
    /**
     * Checks if device is iOS device
     * Source: https://gist.github.com/rproenca/64781c6a1329b48a455b645d361a9aa3#file-clipboard-js-L5
     */
    is_iOS: function () {
    ...
    /**
     * Selects the text in a given text area element
     * Source: https://gist.github.com/rproenca/64781c6a1329b48a455b645d361a9aa3#file-clipboard-js-L15
     * @param {*} textArea
     */
    selectText: function (textArea) {
      if (this.is_iOS()) {
        window.getSelection().removeAllRanges();
    ...
    /**
     * Iterates through the input coordinate textareas and returns a string of coordinates
     * @param {*} currentText
     */
    appendAllCoordsText: function (currentText) {
    ...
    cpBtnWasClicked: function (evt) {
      ...
      var originalCoord = this.coordtext.value;
      // Check if copy all is initiated
      if (this.input) {
        this.coordtext.value = this.appendAllCoordsText(this.coordtext.value);
      }
      // Select coordinates to copy
      this.selectText(this.coordtext);
      // Execute copy to clipboard
      var result = this.execCopy();
      // Set the input coordinate textarea to the original coordinates if
      // the copy all button was clicked
      if (this.input) {
        this.coordtext.value = originalCoord;
      }
      // Show message if copy was successful
      this.showToolTip(this.cpbtn.id, result ? this.nls.copySuccessful : this.nls.copyFailed);
      // Clear any selected text
      window.getSelection().removeAllRanges();
    },
