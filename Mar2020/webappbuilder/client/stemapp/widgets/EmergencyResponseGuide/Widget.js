...
define([
  ...
  function (
    ...
    return declare([BaseWidget, dijitWidgetBase, dijitWidgetsInTemplate], {
       ...
       */
      onClose: function () {
        //Check if coordinate format dialog is visible. If so, then close it.
        if (this.ERGCoordinateFormat && this.ERGCoordinateFormat.domNode.offsetParent) {
          dijit.popup.close(this.ERGCoordinateFormat);
        }
      },
      ...
      _CreateERGButtonClicked: function () {
        if (this._selectedMaterial && this.ERGCoordTool.inputCoordinate.coordinateEsriGeometry) {
          ...
          if (this._selectedMaterial.hasOwnProperty("TABLE3") && this._selectedMaterial.TABLE3.toLowerCase() === "yes") {
            //Add size check
