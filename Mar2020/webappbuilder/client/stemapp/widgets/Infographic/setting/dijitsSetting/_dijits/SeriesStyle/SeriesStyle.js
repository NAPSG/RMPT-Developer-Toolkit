define([
  ...
  function(declare, on, lang, html, templateString, SeriesStyleItem, SeriesStyles, CustomColor, Evented,
    _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, utils, jimuUtils) {
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      _renderByComputed: function(modle) {
        ...
        var computed = modle.computed;
        if (utils.isEqual(preComputed, computed)) {
          return;
        }
        // update the input string of series radio
        if (!utils.isEqual(computed.chartType, preComputed.chartType)) {
          this._updateSeriesRadioString(computed.chartType);
        }
        // is number type  of custom color item
        if (!utils.isEqual(computed.categoryTypes, preComputed.categoryTypes)) {
          if (this.customColorDijit) {
            this.customColorDijit.setCategoryType(computed.categoryTypes);
          }
        }
        // update custom color select option
        if (!utils.isEqual(computed.customColorSelects, preComputed.customColorSelects)) {
          if (this.customColorDijit) {
            this.customColorDijit.setCustomColorSelectOption(computed.customColorSelects);
          }
        }
        // show radio panel
        if (!utils.isEqual(computed.otherRadio, preComputed.otherRadio)) {
          this._switchRadioDisplay(computed.otherRadio);
        }
        //show color list
        if (!utils.isEqual(computed.colorDisplayMode, preComputed.colorDisplayMode) ||
          !utils.isEqual(computed.otherRadio, preComputed.otherRadio)) {
        ...
        //show opacity
        if (!utils.isEqual(computed.showOpacity, preComputed.showOpacity)) {
        ...
        if (!utils.isEqual(computed.colorSingleMode, preComputed.colorSingleMode)) {
        ...
        if (!utils.isEqual(computed.customToggleIconState, preComputed.customToggleIconState)) {
      ...
      _renderByConfig: function(modle) {
        ...
        if (utils.isEqual(config, preConfig)) {
          return;
        }

        if (!utils.isEqual(config.type, preConfig.type)) {
        ...
        //styles
        if (!utils.isEqual(config.styles, preConfig.styles)) {
        ...
        // customColor
        if (!utils.isEqual(config.customColor, preConfig.customColor)) {
      ...
      _updateSeriesRadioString: function(type) {
        if (type === 'pie') {
          ...
          this.setColorLabel.title = this.nls.randomColor;
        } else {
          this.setColorLabel.innerHTML = this.nls.setColor;
          this.setColorLabel.title = this.nls.setColor;
        }
      },
