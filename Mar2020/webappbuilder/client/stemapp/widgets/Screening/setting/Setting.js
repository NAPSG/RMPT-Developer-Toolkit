    ...
    */
    ...
    _setDownloadInfo: function (tr) {
      ...
      if (this._downloadTab) {
        ...
        if (selectedLayerInfo) {
          requestHandle = esriRequest({ "url": selectedLayerInfo.url + "?f=pjson" });
    ...
    _populateSelectableLayer: function () {
      array.forEach(this._totalLayers, lang.hitch(this, function (layerObject) {
        ...
        if (this.config && this.config.drawToolSelectableLayers &&
          (this.config.drawToolSelectableLayers.indexOf(layerObject.id) > -1)) {
