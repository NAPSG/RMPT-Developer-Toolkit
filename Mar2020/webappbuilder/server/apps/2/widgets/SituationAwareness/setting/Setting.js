...
define([
    ...
    function (
        declare, array, lang, Color, html, domStyle, all, Deferred, Evented, on, query, domAttr, string,
        Select, ValidationTextBox, _WidgetsInTemplateMixin, focusUtil,
        FieldPicker, esriLang, FeatureLayerChooserFromMap, jimuUtils, BaseWidgetSetting,
        Message, Popup, TabContainer3, GpSource, LayerInfos,
        CheckBox, domClass, domConstruct, keys, registry, ImageChooser, portalUtils, LayerChooserFromMapWithDropbox
    ) {
        return declare([BaseWidgetSetting, _WidgetsInTemplateMixin, Evented], {
            ...
            postCreate: function () {
                ...
                this.themeLayerTable.onBeforeRowDelete = lang.hitch(this, this._onBeforeDeleteTheme);

                this._addValidator(this.theme_label, this.checkSmallString, this.nls.invalid_string_width);
                ...
                this.own(on(this.btnAddTheme, 'click', lang.hitch(this, this._addTheme)));
            ...
            _initCheckBoxes: function () {
                ...
                this.chk_csv = this._initCheckBox(this.nls.csv_checkbox, this.div_chk_csv);
            ...
            _initLayerChooser: function () {
                ...
                all(defList).then(lang.hitch(this, function () {
                    if (this._allLayers.length > 0) {
                        ...
                        this.own(on(this.btnAddLayer, 'click', lang.hitch(this, this._addThemeRow)));
                    } else {
            ...
            _addTheme: function () {
                ...
                var themeTextBox = new ValidationTextBox({
                    style: {
                        width: "100%",
                        height: "26px"
                    }
                ...
                domConstruct.place(themeTextBox.domNode, contentDiv);
                themeTextBox.startup();   
                
                this.optionsPopup = new Popup({
                    titleLabel: "Enter the Theme",
                    width: 300,
                    maxHeight: 600,
                    autoHeight: true,
                    content: contentDiv,
                    buttons: [{
                        ...
                        onClick: lang.hitch(this, function () {
                            var name = themeTextBox.value;

                            this.themes.push({
                                id: this.themes.length,
                                name: themeTextBox.value,
                                layers:[]
                            });
                            this.updateThemes();
            ...
            _editTheme: function (themeIndex, valueBefore) {
                var contentDiv = domConstruct.toDom('<div></div>');
                ...
                themeTextBox.set("value", valueBefore);
                
                this.optionsPopup = new Popup({
                    titleLabel: "Edit the Theme Name",
                    width: 300,
                    maxHeight: 600,
                    autoHeight: true,
                    content: contentDiv,
                    buttons: [{
                        label: 'Ok',
                        onClick: lang.hitch(this, function () {
                            var newName = themeTextBox.value;

                            this.themes[themeIndex].name = newName;
                            this.updateThemes();
                            this.optionsPopup.close();
                        })
                    }, {
            ...
            updateThemes: function () {
                ...
                html.empty(this.themesContent);
                for (var i = 0; i < this.themes.length; i++) {
                    html.create("div", {
                        "class": "item",
                        "data-theme-id": this.themes[i].id,
                        "innerHTML": '<div class="label" data-theme-id="' + this.themes[i].id + '" title="' + this.themes[i].name + '">' + this.themes[i].name + '</div>' +
                            '<div class="actions jimu-float-trailing">' +
                            '<div data-theme-id="' + this.themes[i].id + '" class="delete action jimu-float-trailing"></div>' +
                            '<div data-theme-id="' + this.themes[i].id + '" class="edit action jimu-float-trailing"></div>' +
                            '<div  data-theme-id="' + this.themes[i].id + '" class="down action jimu-float-trailing"></div>' +
                            '<div  data-theme-id="' + this.themes[i].id + '" class="up action jimu-float-trailing"></div>' +
                            '</div>'
                    }, this.themesContent);
                }
            },

            // this is the theme action
            _onListContentClicked: function (event) {
                ...
                if (html.hasClass(target, 'action')) {
                    if (html.hasClass(target, 'edit')) {
                        themeName = this.themes[target.dataset.themeId].name;
                        this._editTheme(target.dataset.themeId, themeName);
                    } else if (html.hasClass(target, 'up')) {
                        if (itemDom.previousElementSibling) {
                            html.place(itemDom, itemDom.previousElementSibling, 'before');
                            var tempTheme = this.themes[parseInt(target.dataset.themeId)];
                            this.themes[target.dataset.themeId] = this.themes[parseInt(target.dataset.themeId) - 1];
                            this.themes[parseInt(target.dataset.themeId) - 1] = tempTheme;
                            this.updateThemes();
                        }
                        ...
                        if (itemDom.nextElementSibling) {
                            ...
                            var tempTheme = this.themes[parseInt(target.dataset.themeId)];
                            this.themes[parseInt(target.dataset.themeId)] = this.themes[parseInt(target.dataset.themeId) + 1];
                            this.themes[parseInt(target.dataset.themeId)+ 1] = tempTheme;
                            
                            this.updateThemes();
                        }
                    } else if (html.hasClass(target, 'delete')) {
                        var id = target.dataset.themeId;
                        var index = -1;
                        for (var i = 0; i < this.themes.length; i++) {
                ...
                else if (itemDom.dataset && itemDom.dataset.themeId) {
                    var trs = this.themeLayerTable.getRows();
                   
                    this._selectedThemeLayerIds = [];
                    this.themeLayerTable.clear();
                    this.selectedTheme = undefined;
                    this.selectedTheme = this.themes.find(x => x.id === parseInt(itemDom.dataset.themeId));
                    if (this.selectedTheme) {
                        this.selectedTheme.layers.forEach(lang.hitch(this, function (id) {
                            this._populateThemeTableRow(id);
                        }));
                    }
                }
            },
            ...
            _initTabs: function () {
                this._tabsContainer = new TabContainer3({
                    tabs: [{
                        ...
                        title: this.nls.theme_options,
                        content: this.themesTab
                    }, {
            ...
            setConfig: function (config) {
                ...
                this.theme_label.set("value", this.config.themeLabel ?
                    this.config.themeLabel : "Lifeline");
                ...
                this.chk_csv.setValue(this.config.csvAllFields);
            ...
            getConfig: function () {
                ...
                this.config.themes = this.themes;
                this.config.themeLabel = this.theme_label.value;
                ...
                this.config.csvAllFields = this.chk_csv.getValue();
            ...
            _populateThemeTableRow: function (id) {
                ...
                if (layerInfo) {
                    var result = this.themeLayerTable.addRow({});
                    if (result.success && result.tr) {
                        var tr = result.tr;
                        tr.tabInfo = id;
                        this._addThemeLayers(tr);
            ...
            _onBeforeDeleteTheme: function (row) {
                this._selectedThemeLayerIds.splice(row.rowIndex, 1);
                this.selectedTheme.layers.splice(row.rowIndex, 1);
                return true;
            },
            ...
            _getThemeLayerList: function () {
                var layersIds;
                if (this._selectedThemeLayerIds.length < this._allLayers.length) {
                    layersIds = array.filter(this._allLayers, lang.hitch(this, function (layer) {
                        return this._selectedThemeLayerIds.indexOf(layer.id) < 0;                       
                    }));
                }
                else {
            ...
            _addThemeRow: function () {
                var result = this.themeLayerTable.addRow({});
                if (result.success && result.tr) {
                    var tr = result.tr;
                    var layerList = this._getThemeLayerList();
                    this._addThemeLayers(tr, layerList[0]);
                }
            },

            _addThemeLayers: function (tr, layerInfo) {
                ...
                if (td) {
                    ...
                    var themeLayers = new LayerChooserFromMapWithDropbox({
                    ...
                    themeLayers.placeAt(td);
                    themeLayers.startup();
                    tr.selectLayers = themeLayers;
                    themeLayers._missingMsg = this.nls.layer_error;

                    this.own(on(themeLayers, 'selection-change', function (layer) {
                        this.emit('used-theme-layer-change', { layer: layer[0], rowIndex: this.row.rowIndex });
                    ...
                    this.own(on(themeLayers, 'used-theme-layer-change', lang.hitch(this, function (changedData) {
                        this.selectedTheme.layers[changedData.rowIndex] = changedData.layer.id;
                        this._selectedThemeLayerIds[changedData.rowIndex] = changedData.layer.id;
                    })));

                    if (layerInfo) {
                        layerInfo.getLayerObject().then(lang.hitch(this, function (layer) {
                            this.selectedTheme.layers.push(layer.id);
                            tr.selectLayers.setSelectedLayer(layer);
                        }));
                    }

                    focusUtil.focus(themeLayers.domNode);
                    themeLayers.domNode.blur();
                    var table = query(".jimu-simple-table")[0];
                    focusUtil.focus(table);
                }
            },
