...
define([
  ...
  function (declare, lang, array, html, BaseWidget, query, on, Deferred,
    ...
    return declare([BaseWidget], {
      ...
      resetFocusableClass: function(){
        var bookmarks = [];
        bookmarks = this.customBookmarks.bookmarks.length ? this.customBookmarks.bookmarks :
        this.webmapBookmarks.bookmarks;

        if(bookmarks.length){
          var itemNodeDom = bookmarks[bookmarks.length - 1].itemNode.domNode;
          jimuUtils.initLastFocusNode(this.domNode, itemNodeDom);
        }else{
          jimuUtils.initLastFocusNode(this.domNode, this.headBar.addBtn);
        }
      },
