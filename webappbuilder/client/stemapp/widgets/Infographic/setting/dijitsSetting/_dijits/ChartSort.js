  ...
  function(declare, query, lang, html, templateString, Evented,
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      _render: function(modle) {
        ...
        this._dynamicUpdateConfig(modle);
      ...
      _dynamicUpdateConfig: function(modle) {
        var config = modle.config;
        var computed = modle.computed;
        if (!config || !computed) {
          return;
        }
        var fieldOption = computed.fieldOption;
        var field = fieldOption && fieldOption[0] && fieldOption[0].value;
        if (!field) {
          return;
        }
        var mode = computed.mode;
      ...
      _renderByComputed: function(modle) {
        ...
        //fieldOption
        if (!utils.isEqual(computed.fieldOption, preComputed.fieldOption)) {
          if (computed.fieldOption && computed.fieldOption.length > 0) {
            utils.updateOptions(this.sortField, computed.fieldOption);
          }
        }

        //sortArrowPosition
        if (!utils.isEqual(computed.sortArrowPosition, preComputed.sortArrowPosition) ||
          !utils.isEqual(computed.mode, preComputed.mode)) {
      ...
      _renderByConfig: function(modle) {
        ...
        //isLabelAxis
        if (!utils.isEqual(config.isLabelAxis, preConfig.isLabelAxis)) {
          this.xAxisRadio.setChecked(config.isLabelAxis);
          this.yAxisRadio.setChecked(!config.isLabelAxis);
        }

        //isAsc
        if (!utils.isEqual(config.isAsc, preConfig.isAsc)) {
          if (config.isAsc) {
            this._selectAscBtn();
          } else {
            this._selectDescBtn();
        ...
        //field
        if (!utils.isEqual(config.field, preConfig.field)) {
          if (config.field) {
